...
def main():
    ...
    max_length = length - padding_length
    for i in range(n_series):
        ...
        # Randomly truncate the series
        l = np.random.randint(max_length - 20, max_length + 1)
        series_i = series_i[:l]

        series.append(series_i)

    # Pad the shorter sequences to a common length
    series = np.array([np.pad(s, (0, max_length - len(s)), mode='constant') for s in series])
    ...
    # Calculating average series with DBA (assuming you have this function implemented separately)
